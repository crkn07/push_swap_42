...
/*   Updated: 2023/04/18 18:29:15 by crtorres         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	what_type_rotate_ba(t_stack *stack_a, t_stack *stack_b)
{
	...
	tmp = stack_b;
	i = times_reverse_rotate_stacks(stack_b, stack_a, stack_b->nbr, 'A');
	//!printf("entra\n");
	while (tmp)
	{
		if (i > times_rotate_stacks(stack_b, stack_a, tmp->nbr, 'A'))
			i = times_rotate_stacks(stack_b, stack_a, tmp->nbr, 'A');
		if (i > times_reverse_rotate_stacks(stack_b, stack_a, tmp->nbr, 'A'))
			i = times_reverse_rotate_stacks(stack_b, stack_a, tmp->nbr, 'A');
